...
#include <string>
#include <stdio.h>
/// Handmade es_json parser. Goal: optimized for elasticsearch.
/// Must be fast (0 copy parser, etc.)

namespace es_json {

/**
  es_json Class to parse incoming elasticsearch messages:
...
class Value {
    public:
        ...
        void setObject(const es_json::Object& obj);

        /// Set this value as Array.
        void setArray(const es_json::Array& array);
        ...
        static bool weakEquality(const es_json::Value& a, const es_json::Value& b);
        ...
        // Output es_json Value in a pretty format.
...
class Object {
    public:
        ...
        void addMemberByKey(const std::string& key, const es_json::Array& array);
        void addMemberByKey(const std::string& key, const es_json::Object& obj);
        void addMemberByKey(const std::string& key, const es_json::Value& value);
...
class Array {
    public:
        ...
        void addElement(const es_json::Value& val);

        /// Copy the object to a value and add this value to the list.
        void addElement(const es_json::Object& obj);
        ...
        // const int get_sentence_id() { return (*_it).getObject().getValue("_source").getObject().getValue("sentence_id");}
        // const std::string get_sentence() { return (*_it).getObject().getValue("_source").getObject().getValue("sentence");}
        // const string get_doc_name() { return (*_it).getObject().getValue("_source").getObject().getValue("doc_name");}
